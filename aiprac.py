...
class Dqn:
    ...
    def __init__(self, input_size, nb_action, gamma):
        ...
        self.memory = ReplaylayMemory(100000)
    ...
    def select_action(self, state):
        probs = F.softmax(self.model.forward(Variable(state, volatile = True))*100)
        action = probs.multinomial()
        return action.data[0,0]
    
    def learn(self, batch_state, batch_next_state, batch_reward, batch_action):
        outputs = self.model.forward(batch_state).gather(1, batch_action.unsqueeze(1)).squeeze(1)
        next_output = self.model.forward(batch_next_state).detach().max(1)[0]
